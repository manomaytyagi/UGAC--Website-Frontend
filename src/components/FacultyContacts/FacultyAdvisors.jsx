...
// Read both spellings so a backend rename does not break the page either way.
const listOf = (branch) => branch.advisors || branch.advisers || [];

function buildFallback() {
  return ESTABLISHED.map(([code, name], bi) => {
    const meta = branchMeta(code);
    const n = (bi % 4) + 1;
    const advisors = Array.from({ length: n }, (_, i) => ({
    ...
    return { id: code, code, name: meta.name || name, color: meta.color, advisors, count: advisors.length };
  });
}
...
function SourceBadge({ source }) {
  if (source === "loading")
    return <div className="cmx-note"><span className="cmx-badge cmx-badge--load">Loading…</span> Fetching advisors from the API.</div>;
  if (source === "live")
    return <div className="cmx-note"><span className="cmx-badge cmx-badge--live">Live</span> Showing the live advisor roster from the API.</div>;
...
function AdvisorCard({ entry, color, branchName }) {
...
function BranchDetail({ branch, branches, onBack, onSwitch }) {
  ...
  const advisors = listOf(branch);
  const count = branch.count ?? advisors.length;
  return (
      ...
      <div className="cm-adv-head">
        ...
        <div>
          <span className="cm-eyebrow" style={{ color: c }}>
            Faculty advisors · {count} {count === 1 ? "advisor" : "advisors"}
          </span>
      ...
      <div className="cm-adv-grid">
        <div>
          ...
          <div className="cm-year-grid">
            {advisors.length === 0 && (
              <div className="cm-adv-empty">No faculty advisors listed for this branch yet.</div>
            )}
            {advisors.map((a, i) => (
              <AdvisorCard key={i} entry={a} color={c} branchName={branch.name} />
            ))}
          </div>
        </div>
  ...
  return (
    <div>
      ...
      <div className="cm-branch-grid">
        {branches.map((b) => {
          const count = b.count ?? listOf(b).length;
          return (
            <button
              key={b.id}
              type="button"
            ...
            >
              ...
              <span className="cm-branch-cta">View advisors <span aria-hidden>→</span></span>
            </button>
          );
        })}
      </div>
