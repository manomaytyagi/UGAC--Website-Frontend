// Shared building blocks for the faculty contacts section.
...
for (let s = 0; s < 3; s += 1) {
  for (let h = 0; h < 4; h += 1) BRANCH_COLORS.push(HUES[HUE_ORDER[h]][s]);
}
...
export function Contacts({ member, accent = C.navyLight, large = false, onDark = false, compact = false }) {
  ...
  const chip = (k, href, icon, text, target) => (
    <a
      ...
      className={"cm-chip" + (onDark ? " is-dark" : "")}
      style={{ "--cc": accent, fontSize: fs }}
    >
